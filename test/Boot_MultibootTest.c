...
static void Boot_MultibootTest_scanModules_callback(void *closure, size_t index, PhysicalAddress begin, PhysicalAddress end, PhysicalAddress name) {
...
static void Boot_MultibootTest_scanModules() {
    ...
    size_t callCount = 0;
    MultibootMbi_scanModules(&mbi, &callCount, Boot_MultibootTest_scanModules_callback);
    ASSERT(callCount == 3);
}

static void Boot_MultibootTest_scanModulesWithNoModulesProvided() {
...
static void Boot_MultibootTest_scanMemoryMap_callback(void *closure, uint64_t begin, uint64_t length, int type) {
    size_t *callCount = (size_t *) closure;
    ASSERT(*callCount < 3);
...
static void Boot_MultibootTest_scanMemoryMap() {
    ...
    size_t callCount = 0;
    MultibootMbi_scanMemoryMap(&mbi, &callCount, Boot_MultibootTest_scanMemoryMap_callback);
    ASSERT(callCount == 3);
}

static void Boot_MultibootTest_scanMemoryMapWithMemoryMapNotProvided_callback(void *closure, uint64_t begin, uint64_t length, int type) {
    size_t *callCount = (size_t *) closure;
    ASSERT(*callCount < 2);
...
static void Boot_MultibootTest_scanMemoryMapWithMemoryMapNotProvided() {
    MultibootMbi mbi = { .flags = MULTIBOOTMBI_MEMLOWER_MEMUPPER_PROVIDED, .mem_lower = 640, .mem_upper = 1047552 };
    size_t callCount = 0;
    MultibootMbi_scanMemoryMap(&mbi, &callCount, Boot_MultibootTest_scanMemoryMapWithMemoryMapNotProvided_callback);
    ASSERT(callCount == 2);
}

static void Boot_MultibootTest_scanMemoryMapWithNoMemoryInfoProvided() {
    MultibootMbi mbi = { .flags = 0 };
    size_t callCount = 0;
    MultibootMbi_scanMemoryMap(&mbi, &callCount, NULL);
    ASSERT(callCount == 0);
}
static void Boot_MultibootTest_scanElfSections_callback(void *closure, size_t index, PhysicalAddress begin, PhysicalAddress end) {
    size_t *callCount = (size_t *) closure;
    ASSERT(*callCount < 3);
...
static void Boot_MultibootTest_scanElfSections() {
    ...
    size_t callCount = 0;
    MultibootMbi_scanElfSections(&mbi, &callCount, Boot_MultibootTest_scanElfSections_callback);
    ASSERT(callCount == 3);
}

static void Boot_MultibootTest_scanElfSectionsWithNoSectionsProvided() {
    MultibootMbi mbi = { .flags = 0 };
    size_t callCount = 0;
...
void Boot_MultibootTest_run() {
    RUN_TEST(Boot_MultibootTest_scanModules);
    RUN_TEST(Boot_MultibootTest_scanModulesWithNoModulesProvided);
    RUN_TEST(Boot_MultibootTest_scanMemoryMap);
    RUN_TEST(Boot_MultibootTest_scanMemoryMapWithMemoryMapNotProvided);
    RUN_TEST(Boot_MultibootTest_scanMemoryMapWithNoMemoryInfoProvided);
    RUN_TEST(Boot_MultibootTest_scanElfSections);
    RUN_TEST(Boot_MultibootTest_scanElfSectionsWithNoSectionsProvided);
}
