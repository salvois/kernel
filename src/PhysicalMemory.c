...
static Task dummyTaskToIdentifyFreeFrames;
...
void PhysicalMemoryRegion_add(PhysicalMemoryRegion *pmr, FrameNumber begin, FrameNumber end) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (frame->task != &dummyTaskToIdentifyFreeFrames) {
            frame->virtualAddress = VIRTUAL_ADDRESS_UNMAPPED;
            frame->task = &dummyTaskToIdentifyFreeFrames;
...
FrameNumber PhysicalMemoryRegion_allocate(PhysicalMemoryRegion *pmr, Task *task) {
    ...
    assert(frame->task == &dummyTaskToIdentifyFreeFrames);
    frame->task = task;
    frame->virtualAddress = VIRTUAL_ADDRESS_UNMAPPED;
